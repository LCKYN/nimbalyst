...
- Codex file tracking no longer lists identically rebuilt files as edits, keeps tracking after a slow build or when another agent session is active in the same workspace, no longer reports an interruption after a normal restart, no longer warns about a command that ended without touching any file, and ignores tool activity from Codex subagents instead of attributing their writes to the parent session.
