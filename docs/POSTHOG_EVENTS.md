...
| `ai_prompt_scheduled` | `SessionTranscript.tsx` | User schedules the composer's prompt with "Run later" and the schedule is saved | `choice` (in_1h/in_4h/tomorrow_morning/usage_reset/custom)<br/>`lead_time` (<1h/1-4h/4-12h/12-24h/>24h)<br/>`has_attachments` (boolean)<br/>`provider` (string) | (pending release) |  |
